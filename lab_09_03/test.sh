...
do
    ...
    if [[ $1 == "-m" ]]
    then
        valgrind -q --log-fd=9 9>log/valg${index}_pos.txt --leak-check=full ./app.exe $line >output.txt
    else
        ./app.exe $line >output.txt
    fi
...
do
    ...
    if [[ $1 == "-m" ]]
    then
        valgrind -q --log-fd=9 9>log/valg${index}_neg.txt --leak-check=full ./app.exe $line >output.txt
    else
        ./app.exe $line >output.txt
    fi
