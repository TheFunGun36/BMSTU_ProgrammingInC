...
string_part get_word_last(string_part source_str, char splitter)
{
    ...
    while (*word.end == splitter && word.end > source_str.begin)
        word.end--;

    word.begin = word.end++;

    while (*word.begin != splitter && word.end > source_str.begin)
        word.begin--;

    if (*word.begin == splitter)
        return get_empty_string();
    else
        return word;
}
...
void remove_symbol(string_part *str, char symbol)
{
    ...
    for (char *p_old = str->begin; p_old < str->end; p_old++)
    {
        if (*p_old != symbol)
        {
            *p_new++ = *p_old;
